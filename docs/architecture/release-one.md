...
## Phase 8 — Release, CI, and signed updater (complete, bar the key)
...
`pubkey` is `REPLACE_WITH_UPDATER_PUBLIC_KEY`. The app builds and runs; **update
checks fail** until it is replaced — the safe way round. Generating the real key
pair is a manual step for the maintainer, documented in `docs/release.md`, and
deliberately not done here: the private half signs anything every install will
run.
...
Two items, both needing something only the maintainer can do:

- **The updater key pair is not generated.** `pubkey` is
  `REPLACE_WITH_UPDATER_PUBLIC_KEY`; update checks fail until it is replaced,
  which is the safe way round. The private half signs anything every install
  will run, so generating it is a manual step — see `docs/release.md`.
- **Neither workflow has been run.** That needs a push to GitHub.
